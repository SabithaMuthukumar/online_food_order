...
$customer_address = '';
$grand_total = $_SESSION['order_details']['grand_total'] ?? 0;
...
$deliveryPersonQuery = $conn->prepare("SELECT Del_person_id, Name FROM Delivery_persons WHERE Status = 'available'");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        $orderStmt = $conn->prepare("INSERT INTO Orders (Ord_number, Cust_id, Ord_date, Status, Total_Tax, Delivery_charge, Grand_Tot) 
                                    VALUES (?, ?, ?, ?, ?, ?, ?)");
        $orderStmt->bind_param("ssssddd", $orderNumber, $_SESSION['customer_id'], $orderDate, $status, 
            ...
            if (empty($errors)) {
                if ($payment_method === 'cash') {
                    ...
                    if ($deliveryStmt->execute()) {
                        ...
                        // Insert into Trans table for cash payment
                        $transStmt = $conn->prepare("INSERT INTO Trans (Trans_id, Ord_id, Pay_Mode, Pay_Status, Total_paid_amt) 
                                                    VALUES (?, ?, 'Cash on Delivery', 'Not Completed', ?)");
                    ...
                    if (isset($_POST['transaction_id']) && isset($_POST['full_name'])) {
                        ...
                        if (empty($transaction_id) || empty($full_name)) {
                            ...
                            // Insert into Trans table for mobile payment
                            $transStmt = $conn->prepare("INSERT INTO Trans (Trans_id, Ord_id, Pay_Mode, Pay_Status, Total_paid_amt) 
                                                        VALUES (?, ?, 'Mobile Banking', 'Completed', ?)");
                            $transStmt->bind_param("sid", $trans_id, $ord_id, $orderDetails['grand_total']);
...
<body>
            ...
            <div class="confirmation">
                ...
                <div class="delivery-info">
                    ...
                    <?php if ($delivery_person_name !== 'Not assigned yet'): ?>
                        <p>Your order will be delivered by <strong><?php echo $delivery_person_name; ?></strong></p>
                    <?php else: ?>
  ...
  <script>
    ...
    function processCashPayment() {
        ...
        // Submit the form with cash payment method
        document.getElementById('paymentMethod').value = 'cash';
        document.getElementById('paymentForm').submit();
    }
    
    // Validate mobile payment form before submission
    document.getElementById('paymentForm').addEventListener('submit', function(e) {
        ...
        if (paymentMethod === 'mobile') {
            ...
            if (!transactionId || !fullName) {
                ...
                // Show processing overlay for mobile payment
                document.getElementById('processingOverlay').style.display = 'flex';
            }
        }
    });

    // Hide processing overlay when page loads
