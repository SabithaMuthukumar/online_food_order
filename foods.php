...
// Initialize last ordered items if not exists
if (!isset($_SESSION['last_ordered_items'])) {
    $_SESSION['last_ordered_items'] = array();
}

// Handle setting last order from localStorage
if (isset($_POST['action']) && $_POST['action'] == 'set_last_order' && isset($_POST['last_order'])) {
    $lastOrder = json_decode($_POST['last_order'], true);
    if (is_array($lastOrder)) {
        $_SESSION['last_ordered_items'] = $lastOrder;
    }
    echo 'OK';
    exit();
}
...
if (isset($_GET['category_id']) && !empty($_GET['category_id'])) {
    ...
    $category_filter = " AND Cat_ID = $category_id";
    
    // Get category name for display
    $cat_sql = "SELECT Title FROM categories WHERE Cat_ID = $category_id";
...
// Fetch food items from database with optional category filter
$sql = "SELECT * FROM Foods WHERE Active = 'Yes' $category_filter";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['place_order'])) {
        ...
        foreach ($_SESSION['cart'] as $item) {
            ...
            // Calculate individual item values (subtotal does NOT include tax)
            $unitPrice = $price;
            $discountAmountPerUnit = $price * $discountPercent / 100;
            $taxAmountPerUnit = $price * $taxPercent / 100;
            $finalPricePerUnit = $unitPrice - $discountAmountPerUnit;

            // Calculate totals
            $itemSubtotal = $finalPricePerUnit * $quantity;
            $totalItemTax = $taxAmountPerUnit * $quantity;
...
// Get food suggestions based on items in cart
...
if (!empty($_SESSION['cart'])) {
    ...
    foreach ($_SESSION['cart'] as $item) {
        ...
        if (isset($foodItems[$foodId])) {
            $cartCategories[] = $foodItems[$foodId]['Cat_id'];
        }
    }
    ...
    // If we have categories, get other foods from those categories
    if (!empty($cartCategories)) {
        $categoryList = implode(',', $cartCategories);
        $suggestionSql = "SELECT * FROM Foods WHERE Active = 'Yes' AND Cat_id IN ($categoryList) LIMIT 6";
        $suggestionResult = $conn->query($suggestionSql);
        
        if ($suggestionResult && $suggestionResult->num_rows > 0) {
    ...
    <section class="food-menu">
        <div class="container">
            ...
            <div class="category-header">
                ...
                <a href="foods.php" class="clear-filter">Show All Foods</a>
                <?php endif; ?>
            </div>
            ...
            <div style="text-align:center; margin-bottom: 30px;">
                <input type="text" id="foodSearch" placeholder="Search for food..." style="width: 60%; padding: 12px; font-size: 1.1rem; border-radius: 8px; border: 1px solid #ccc;">
            </div>
            ...
            <div class="food-grid" id="foodGrid">
                <?php if (count($foodItems) > 0): ?>
                    ...
                    <div class="food-card" data-food-name="<?php echo htmlspecialchars(strtolower($food['Food_name'])); ?>" data-food-desc="<?php echo htmlspecialchars(strtolower($food['Description'])); ?>">
                ...
                <?php else: ?>
                    <div style="grid-column: 1 / -1; text-align: center; padding: 40px;">
                        ...
                        <a href="categories.php" class="btn btn-primary">Browse Categories</a>
                    </div>
                <?php endif; ?>
            </div>
    ...
    <div id="suggestionsModal" class="modal">
        <div class="modal-content">
            ...
            <div class="suggestions-grid">
                <?php if (!empty($suggestedFoods)): ?>
                    <?php foreach ($suggestedFoods as $suggested): ?>
                    <div class="suggestion-item">
                        <img src="<?php echo $suggested['Image']; ?>" alt="<?php echo $suggested['Food_name']; ?>" class="suggestion-img">
                        <h4><?php echo $suggested['Food_name']; ?></h4>
                        <p class="food-price">
                            <?php if ($suggested['Discount_percent'] > 0): ?>
                            <span class="discounted-price">&#8377;<?php 
                                $discountedPrice = $suggested['Price'] * (1 - $suggested['Discount_percent']/100);
                                echo number_format($discountedPrice, 2);
                            ?></span>
                            <?php else: ?>
                            <span class="no-discount-price">&#8377;<?php echo number_format($suggested['Price'], 2); ?></span>
                            <?php endif; ?>
                        </p>
                        <button class="btn btn-primary add-suggestion" data-food-id="<?php echo $suggested['Food_id']; ?>">Add to Cart</button>
                    </div>
                    <?php endforeach; ?>
                <?php else: ?>
...
<script>
    ...
    function saveCartToStorage() {
        const lastOrdered = <?php echo json_encode($_SESSION['last_ordered_items']); ?>;
        localStorage.setItem('lastOrderedItems', JSON.stringify(lastOrdered));
        localStorage.setItem('lastOrderTimestamp', new Date().getTime());
    }

    // Function to load cart from localStorage
    function loadCartFromStorage() {
        const lastOrdered = localStorage.getItem('lastOrderedItems');
        const timestamp = localStorage.getItem('lastOrderTimestamp');
        
        if (lastOrdered && timestamp) {
            // Check if data is less than 30 days old
            const thirtyDaysAgo = new Date().getTime() - (30 * 24 * 60 * 60 * 1000);
            if (parseInt(timestamp) > thirtyDaysAgo) {
                return JSON.parse(lastOrdered);
            } else {
                // Clear expired data
    ...
    // Function to clear stored cart data
    ...
    function filterFoods() {
        const searchInput = document.getElementById('foodSearch').value.trim().toLowerCase();
        const foodCards = document.querySelectorAll('#foodGrid .food-card');
        let anyVisible = false;
        foodCards.forEach(card => {
            const name = card.getAttribute('data-food-name') || '';
            const desc = card.getAttribute('data-food-desc') || '';
        ...
        const noResultId = 'noFoodFoundMsg';
        let noResultElem = document.getElementById(noResultId);
        if (!anyVisible) {
            if (!noResultElem) {
                noResultElem = document.createElement('div');
                noResultElem.id = noResultId;
                noResultElem.style.gridColumn = '1 / -1';
                noResultElem.style.textAlign = 'center';
                noResultElem.style.padding = '40px';
                noResultElem.innerHTML = '<h3>No food items found.</h3><p>Try a different search.</p>';
                document.getElementById('foodGrid').appendChild(noResultElem);
            }
        } else {
            if (noResultElem) noResultElem.remove();
        }
    }
    ...
    // Function to add suggested food to cart
    function addSuggestionToCart(foodId) {
        const quantityElement = document.getElementById(`qty-${foodId}`);
        let currentQuantity = parseInt(quantityElement.textContent);
        ...
        // Wait for cart update before refreshing summary
        updateCart(foodId, newQuantity).then(() => {
            updateOrderSummary();
        });
    }
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        placeOrderBtn.addEventListener('click', function () {
            ...
            setTimeout(showFoodSuggestions, 500);
        });
        ...
        closeSuggestionsModal.addEventListener('click', function () {
        ...
        document.querySelectorAll('.add-suggestion').forEach(button => {
            button.addEventListener('click', function() {
                ...
                addSuggestionToCart(foodId);
            });
        });
    });

    // Save to localStorage when order is placed
