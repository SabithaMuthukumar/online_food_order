...
if (isset($_POST['save_status']) && isset($_POST['status'])) {
    $delId = (int)$_POST['save_status'];
    $newStatus = $_POST['status'][$delId] ?? '';
    $allowed = ['Pending', 'Processing', 'Shipped', 'Delivered', 'Cancelled'];
    if (in_array($newStatus, $allowed)) {
        $stmt = $conn->prepare("UPDATE delivery SET Del_status = ? WHERE Del_ID = ?");
        $stmt->bind_param("si", $newStatus, $delId);
        $stmt->execute();
        $stmt->close();
        ...
        <table class="styled-table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td><?php echo is_null($row['delivery_person_name']) ? 'null' : htmlspecialchars($row['delivery_person_name']); ?></td>
                    <td>
                        <select name="status[<?php echo $row['Del_ID']; ?>]">
                            <?php
                            $statuses = ['Pending', 'Processing', 'Shipped', 'Delivered', 'Cancelled'];
                            foreach ($statuses as $status) {
                                $selected = ($row['Del_status'] === $status) ? 'selected' : '';
                    ...
                    <td class="actions">
                        <button type="submit" name="save_status" value="<?php echo $row['Del_ID']; ?>" class="btn btn-primary">Save</button>
                    </td>
                </tr>
                <?php endwhile; ?>
            </tbody>
